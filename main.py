...
# Načítaj DICOM súbor, Funkcie bytvori objekt, ktory obsahuje data aj metadata
...
# Získaj počet snímok, Zistije sa ti je snimka 3D
...
# Vytvor podokná pre zobrazenie snímok (prispôsobí počet riadkov/colí podľa počtu snímok)
    cols = 5  # počet stĺpcov v mriežke
    rows = (num_slices // cols) + (num_slices % cols > 0) # Vypocitanie poctu riadkov na zaklade poctu snimkov

    fig, axes = plt.subplots(rows, cols, figsize=(cols * 4, rows * 4)) # Vytvorenie mriezky a nadstavi velkost kazdeho podgrafu 4x4 palcov
    axes = axes.flatten()  # Plochý jednorozmerny zoznam
